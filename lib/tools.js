...
import { runOrrJson } from "./runner.js";
import { generateSkillSkeleton, installSkill, resolveSkillTarget, summarizeEvidence, validateSkillDraft, validateSkillName, } from "./skillCreate.js";
...
function truncate(text, maxChars = 24_000) {
...
function workspaceOf(exec, fallback) {
    const agent = exec.agent;
    return agent?.session?.header?.cwd ?? fallback;
}
/** Build the six model-facing tools that wrap `bin/orr.js`. */
export function createRecordReplayTools(ctx, options) {
    const { cliPath, runsOut, skillInputsOut } = options;
    const cwdOf = (exec) => workspaceOf(exec, process.cwd());
    const permissionsCheck = defineTool({
        name: 'orr_permissions_check',
        description: 'Check macOS Accessibility and Input Monitoring permissions for the Open '
    ...
    const recordStart = defineTool({
        ...
        description: 'Start an Open Record/Replay session that records the user\'s real macOS '
    ...
    const recordStop = defineTool({
        ...
        description: 'Stop the running Open Record/Replay recording and finalize the session '
    ...
    const sessionEvents = defineTool({
        ...
        description: 'Read the recorded event stream (events.jsonl) for a session: the primary '
    ...
    const sessionValidate = defineTool({
        ...
        description: 'Validate the recording quality of a session against the official '
    ...
    const skillPrepare = defineTool({
        ...
        description: 'Package a recorded session into a skill input package '
    ...
    const skillCreate = defineTool({
        ...
        description: 'Create a reusable skill from a recorded session, following the '
        ...
        async execute(args, exec) {
            ...
            const actions = summary.actions.slice(0, 60).map(a => {
                const plain = { kind: a.kind };
                if (a.app !== undefined)
                    plain.app = a.app;
                if (a.semantic !== undefined)
                    plain.semantic = a.semantic;
                return plain;
            });
            return {
                name,
...
function latestSessionId(sessionsDir) {
