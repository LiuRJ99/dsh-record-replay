...
export function summarizeEvidence(eventsPath) {
...
export function validateSkillName(name) {
    if (typeof name !== 'string' || name.trim() === '')
        return 'skill name must be a non-empty kebab-case string';
...
export function validateSkillDraft(name, draft) {
    const problems = [];
    ...
    if (nameMatch === null || nameMatch[1] === undefined || nameMatch[1].trim() === '') {
        ...
        if (declared !== name)
            problems.push(`frontmatter name "${declared}" does not match requested name "${name}"`);
    ...
    if (body.trim() === '')
        problems.push('SKILL.md body must not be empty');
    return problems;
}
/**
 ...
 */
export function generateSkillSkeleton(summary, name, description) {
...
export function resolveSkillTarget(target, cwd) {
